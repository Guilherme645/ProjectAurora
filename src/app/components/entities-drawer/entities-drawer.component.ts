...
  // Ajuste na lógica para que o modal sempre abra
  openEntityModal(event: MouseEvent, entity: string, type: string): void {
    // Emite um evento de fechamento para garantir que qualquer modal anterior seja fechado
    if (this.currentlyOpenEntity) {
      this.openEntityOptions.emit({ entity: '', type: '', position: { top: 0, left: 0 } });
    }

    // Emite um evento para abrir o novo modal
    const button = event.currentTarget as HTMLElement;
    const rect = button.getBoundingClientRect();
    const modalWidth = 260; // Largura aproximada do modal
...
    // Ajustar a posição para que o modal não saia da tela
    if (left < 0) {
